...
class TestSanitization:
    ...
    def test_remove_null_bytes(self, sanitizer):
        """Null bytes should be removed by sanitize_string_field"""
        # sanitize_string_field removes null bytes and control chars
        result = sanitizer.sanitize_string_field("Hello\x00World", "test_field")
        assert "\x00" not in result

    def test_remove_control_chars(self, sanitizer):
        """Control characters should be removed"""
        result = sanitizer.sanitize_string_field("Hello\x01\x02\x03World", "test_field")
        assert "\x01" not in result
        assert "\x02" not in result
        assert "\x03" not in result

    def test_preserve_newline(self, sanitizer):
        """Newlines should be preserved"""
        result = sanitizer.sanitize_string_field("Hello\nWorld", "test_field")
        assert "\n" in result

    def test_preserve_tab(self, sanitizer):
        """Tabs should be preserved"""
        result = sanitizer.sanitize_string_field("Hello\tWorld", "test_field")
        assert "\t" in result
