...
{
  freelog(LOG_DEBUG, "[INIT] init_sprites() started");
  ...
  if (loaded_images != tileset_image_count) {
    freelog(LOG_DEBUG, "[INIT] Loading " + tileset_image_count + " tileset images");
    ...
    // already loaded
    freelog(LOG_DEBUG, "[INIT] Tileset images already loaded, calling webgl_preload()");
    webgl_preload();
...
{
  loaded_images += 1;
  freelog(LOG_DEBUG, "[INIT] Tileset images loaded: " + loaded_images + "/" + tileset_image_count);

  if (loaded_images == tileset_image_count) {
    freelog(LOG_DEBUG, "[INIT] All tileset images loaded, calling init_cache_sprites() and webgl_preload()");
    init_cache_sprites();
    webgl_preload();
...
function init_cache_sprites()
{
 try {
  ...
  freelog(LOG_NORMAL, "Problem caching sprite: " + tile_tag);
 }
